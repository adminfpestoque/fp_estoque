...
class ProductTest(TestCase):
    ...
    def test_legacy_maximum_stock_is_ignored(self):
        response = self.client.post(
            "/api/products/",
            {
                "code": "REF-003",
                "name": "Produto sem estoque máximo",
                "category": self.category.id,
        ...
        self.assertEqual(response.status_code, 201, response.data)
        self.assertNotIn("maximum_stock", response.data)
        product = Product.objects.get(code="REF-003")
        self.assertEqual(product.minimum_stock, Decimal("20.000"))
        self.assertEqual(product.cost_price, Decimal("0.00"))
    ...
    def test_product_accepts_comma_or_point_for_sale_price_and_ignores_cost(self):
        response = self.client.post(
            "/api/products/",
            {
                "code": "MONEY-001",
        ...
        self.assertEqual(response.status_code, 201, response.data)
        self.assertEqual(response.data["cost_price"], "0.00")
