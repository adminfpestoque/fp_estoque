        ...
        <Modal title={`${form.id ? "Editar" : "Nova"} saída`} onClose={() => !busy && setForm(null)} size="xl">
          <form onSubmit={save} className="checkout-form">
            ...
            <div className="form-actions checkout-actions">
              ...
              <Button type="button" variant="success" onClick={() => save(null, true)} disabled={busy || outputCalculation.invalidStock}>{busy ? "Processando..." : form.id && form.status === "CONFIRMED" ? "Salvar e recalcular" : "Finalizar saída"}</Button>
            </div>
          </form>
        </Modal>
      )}
