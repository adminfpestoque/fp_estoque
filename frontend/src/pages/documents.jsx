...
export function DocumentPage({ type, notify, me }) {
  ...
  const newItem = isEntry ? { product: "", quantity: "1", unit_cost: "0", lot_number: "", manufacturing_date: "", expiration_date: "", notes: "" } : { product: "", quantity: "1", lot: "", notes: "" };
  function start() { setForm(isEntry ? { supplier: "", entry_date: new Date().toISOString().slice(0, 16), invoice_number: "", notes: "", status: "DRAFT", items: [{ ...newItem }] } : { output_date: new Date().toISOString().slice(0, 16), reason: "COMMERCIAL", notes: "", status: "DRAFT", items: [{ ...newItem }] }); }
